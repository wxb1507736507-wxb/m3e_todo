...
| 冷启动耗时 | profile 版实测 `am start -W` TotalTime **1443–1530ms**；分阶段：进程起到 Dart `main()` 约 **420ms**（引擎初始化），`main()` 到 `runApp` 约 **151ms**（其中存储通道 137ms、读设置 13ms），其余为首帧构建 |
...
- **应用背景图**：`RepaintBoundary` + 按显示尺寸解码（见上一节，实测 2 帧掉帧 → 0）。
