...
void main() {
  ...
  group('the network reader', () {
    test('keeps the provider, model and key, and needs all three to be ready', () async {
      final SettingsController controller =
          container.read(settingsProvider.notifier);

      expect(container.read(settingsProvider).visionImportReady, isFalse);
      ...
      final AppSettings stored = await repository().load();
      expect(stored.visionBaseUrl, contains('dashscope'));
      expect(stored.visionModel, 'qwen-vl-max');
      expect(stored.visionApiKey, 'sk-test');
      // Off until switched on: a key alone is not consent to send photos out.
      expect(stored.visionImportEnabled, isFalse);
      expect(stored.visionImportReady, isFalse);

      controller.setVisionImport(enabled: true);
      expect(container.read(settingsProvider).visionImportReady, isTrue);
    });

    test('switched on with no key is not ready', () async {
      final SettingsController controller =
          container.read(settingsProvider.notifier);

      controller.setVisionImport(enabled: true);

      expect(container.read(settingsProvider).visionImportEnabled, isTrue);
      // Otherwise the import would try a call that cannot be made, and report
      // "no API key" as if the photo were at fault.
      expect(container.read(settingsProvider).visionImportReady, isFalse);
    });

    test('a version-6 file loads with the reader unset', () async {
      settingsFile().writeAsStringSync(
        '{"version":6,"themeMode":"dark","artifacts":1,'
        '"widgetBackgroundDim":0.4}',
      );

      final AppSettings stored = await repository().load();

      expect(stored.themeMode, AppThemeMode.dark);
      expect(stored.widgetBackgroundDim, 0.4);
      expect(stored.visionImportEnabled, isFalse);
      expect(stored.visionApiKey, isNull);
      expect(stored.visionImportReady, isFalse);
    });
  });
}
